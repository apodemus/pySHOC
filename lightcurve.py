...
from misc import interleave, groupmore, find_missing_numbers, first_false_idx, sortmore
from myio import iocheck, parsetolist, read_data_from_file, warn
#from superstring import rreplace#kill_brackets   #ProgressBar
from superfits import quickheader
from superplot.lc import lcplot
...
from tsa.spectral import Spectral, Div
...
class Run(object):
    ...
    def __init__(self, filenames, databases=None, coordinates=None,
        ...
        self.cubes = [ Cube(fn, db, cf, target, ref) for (fn, db, cf)  in
                         itt.zip_longest(self.filenames, self.databases, self.coordinates or []) ]           #NOT DYNAMIC!
...
class Cube(object):
    ...
    def __init__(self, filename='', database=None, coordinates=None, target=None, ref=None):
        ...
        if filename:
            self.target_name, self.date, self.size = get_name_date( filename )
    ...
    def fix_ut(self, set_=True):
        ...
        if len(dateset) > 1:
            warn( 'Data spans multiple UTC dates! Adjusting time origin.' )
    ...
    #====================================================================================================
    def load_from_lc(self, filename, mode='flux'):

        (metaline,starline,headline), data = read(filename)
        for i,d in enumerate(Div.div(data.T[1:], 2)):
            if mode == 'mag':
                mag0 = 25
    ...
    def load_from_apcor(self, filename, coofile, splitfile=None ):
        ...
        try:
            ...
            if remainder:
                raise ValueError( 'Dropped Frames!' )
    ...
    def datablock(self, which='raw', with_error=False):
        if with_error:
            return ( np.ma.vstack( [star.get_data(which) for star in self] ),
                     np.ma.vstack( [star.err for star in self] ) )
        else:
            return np.ma.vstack( [star.get_data(which) for star in self] )
    ...
    def compute_dl(self, which='raw', mode='flux', **kw):
        ...
        if mean:
            ...
            comps = self.datablock()[compix,:]         #data for comparison stars
        ...
        if smoothing:    #smoothing gives window size
            ref = smooth(ref, smoothing, fill=None)#, return_type='masked')
    ...
    def compute_ls(self, tkw='utsec', **kw):
        ...
        if not tkw in conjc.timedata.dtype.names:
    ...
    def check_ref(self, ref):
        ...
        if not ref is None:
            if ref != maxsnr:#minvar:
                warn( 'The selected reference star {} is not the one with the best SNR!  Use {} instead.'.format(args.ref, maxsnr) )
            return ref
...
class Star(object):
    ...
    def plot_lc(self, ax, t, which='raw', **kw):
        ...
        fig, plots, *rest = lcplot( t, self.get_data(which), self.err )
    ...
    def clobber_check(filename, clobber):
        if os.path.exists( filename ):
            if clobber is None:
                ...
                return Input.str( msg, 'y', convert=convert.yn2TF )
            return clobber
        else:
            return True
...
#THIS SHOULD BE FITS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
def read(filename):
    header = read_data_from_file( str(filename), N=3 )
...
##############################################################################################################################################
from myio import with_path_conversion, read_file_line
@with_path_conversion
def load_times(timefile):
    '''read timestamps from file.'''

    hline = read_file_line(timefile, 0)
    colnames = hline.strip('\n #').lower().split()

    return np.genfromtxt( timefile,
                            dtype=None,
                            names=colnames,
                            skip_header=1,
...
if __name__=='__main__':
    ...
    args.mags = parsetolist( args.mags, os.path.exists, include=('mag', 'lc'), 
                             path=path, raise_error=1 )
    args.coo = parsetolist( args.coo, os.path.exists, include='coo', 
    ...
    if testfile.exists():
        ...
        for star in conjc:
            ...
            ix = WindowOutlierDetection( star.data, nwindow, noverlap,
                                         generalizedESD, maxOLs=25, alpha=0.05 )
